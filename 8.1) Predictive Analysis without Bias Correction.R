...
##############################################################
# Script: 8.1) Predictive Analysis without Bias Correction
#
# Description:
# For each draw and bias level (sigma), models are trained on the corresponding
# biased training data and used to predict the test data. The RMSE is computed
# to evaluate predictive accuracy across models and bias levels.
##############################################################
...
bias_sigma_list_result_without_IW <- readRDS("biased_data/bias_sigma_list_IW.rds")
...
##### 4) Loop over sigma levels and draws #####

for (sigma in names(bias_sigma_list_result_without_IW)) {
  for (draw in names(bias_sigma_list_result_without_IW[[sigma]])) {
    
    cat(">> Sigma =", sigma, "| Draw =", draw, "\n")
    
    ### Extract training and test data ###
    draw_element <- bias_sigma_list_result_without_IW[[sigma]][[draw]]
    
    train_name <- names(draw_element)[grepl("^train", names(draw_element))]
    training_5 <- draw_element[[train_name]] %>% select(-any_of("importance_weight"))
    
    test_name <- names(draw_element)[grepl("^test", names(draw_element))]
    test <- draw_element[[test_name]] %>% select(-any_of("importance_weight"))
    
    ### Estimate models ###
    
    ## LM_full
    model_LM_full <- gamlss(formula_LM_full, data = training_5, family = NO)
    
    ## LM_red
    model_LM_red <- gamlss(formula_LM_red, data = training_5, family = NO)
    
    ## GAM
    model_GAM <- gamlss(formula_GAM, data = training_5, family = NO)
    
    ## GAMLSS
    model_GAMLSS <- gamlss(formula_GAM, sigma.fo = sigma_formula, data = training_5, family = NO)
    
    ## GAMLSS_lin
    model_GAMLSS_lin <- gamlss(formula_LM_red, sigma.fo = sigma_formula, data = training_5, family = NO)
    
    ## Random Forest
    RF_model <- randomForest(formula = formula_LM_full, data = training_5, ntree = 500)
    
    ## XGBoost
    x_train <- model.matrix(formula_LM_red, data = training_5)[, -1]
    x_test <- model.matrix(formula_LM_red, data = test)[, -1]
    y_train <- training_5$living_space
    ...
    ### Save result table back to list ###
    
    bias_sigma_list_result_without_IW[[sigma]][[draw]]$model_results <- result_df
...
saveRDS(bias_sigma_list_result_without_IW, "biased_data/bias_sigma_list_result_without_IW.rds")
#####
